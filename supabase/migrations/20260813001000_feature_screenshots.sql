-- Noni screenshot library. Each product feature holds many screenshots:
...
comment on table public.feature_screenshots is
  'Screenshot library per product feature. source=noni rows are AI-polished shots sized for phone (1024x1536) or laptop (1536x1024).';
...
-- Ranking now also stamps one video idea per feature so the library card
-- shows what to film with each screenshot.
alter table public.product_features
  add column if not exists idea_title text not null default '',
  add column if not exists idea_example text not null default '',
  add column if not exists idea_action text not null default '';
